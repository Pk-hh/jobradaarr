...
function filterJobs() {
    ...
    let results = allJobs.filter(j => {
        ...
        const isNotExpired = !j.deadline || j.deadline >= now;
    ...
    els.grid.innerHTML = jobs.map((job, i) => {
        ...
        if (job.deadline) {
            ...
            if (daysLeft <= 3 && daysLeft >= 0) {
                deadlineBadge = `<span class="px-2 py-0.5 rounded text-[10px] font-bold bg-red-100 text-red-600 border border-red-200">Values ending soon</span>`;
            } else {
